...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      {view === 'dashboard' && <Dashboard c={c} progress={progress} xp={xp} overall={overall} completed={completed} totalLessons={totalLessons} selectedTrack={selectedTrack} language={language} onStart={() => selectPath('python')} onExplore={() => setView('paths')} onSelectPath={selectPath} />}
      ...
      {view === 'passport' && <Passport c={c} language={language} progress={progress} skills={activeSkills} completed={completed} totalLessons={totalLessons} />}
    </main>
  </div>
}

function Dashboard({ c, progress, xp, overall, completed, totalLessons, selectedTrack, language, onStart, onExplore, onSelectPath }: { c: typeof copy.ar | typeof copy.en; progress: ProgressState; xp: number; overall: number; completed: number; totalLessons: number; selectedTrack: typeof tracks[number]; language: Language; onStart: () => void; onExplore: () => void; onSelectPath: (id: TrackId) => void }) {
  const isAr = language === 'ar'
  return <>
    ...
    <section className="stats-grid" aria-label={c.progress}>
      ...
      <Stat icon={<CheckCircle2 />} value={`${completed}/${totalLessons}`} label={c.lessons} accent="teal" />
    </section>
...
function Paths({ c, language, progress, selectedTrack, selectedLesson, selectedLessonIndex, answers, onSelectTrack, onSelectLesson, onAnswer, onComplete }: { c: typeof copy.ar | typeof copy.en; language: Language; progress: ProgressState; selectedTrack: typeof tracks[number]; selectedLesson: typeof tracks[number]['lessons'][number]; selectedLessonIndex: number; answers: Record<string, number>; onSelectTrack: (id: TrackId) => void; onSelectLesson: (index: number) => void; onAnswer: (index: number) => void; onComplete: () => void }) {
  ...
  const selectedStage = selectedTrack.stages.find((stage) => stage.id === selectedLesson.stage) ?? selectedTrack.stages[0]
  return <>
...
function Passport({ c, language, progress, skills, completed, totalLessons }: { c: typeof copy.ar | typeof copy.en; language: Language; progress: ProgressState; skills: string[]; completed: number; totalLessons: number }) { return <><section className="section-head paths-head"><div><span className="eyebrow"><Award size={15} />IraqCode</span><h1>{c.skillsTitle}</h1><p>{c.skillsText}</p></div></section><section className="passport-card"><div className="passport-header"><div className="passport-mark"><Award size={36} /></div><div><span>{c.proofTitle}</span><h2>IraqCode learner record</h2><p>{completed} {c.lessons} · {progress.completedMissionIds.length} {c.mission}</p></div></div><div className="passport-skills">{skills.length ? skills.map((skill) => <div key={skill}><CheckCircle2 size={16} /><span>{skill}</span><small>{c.verified}</small></div>) : <p className="empty-proof">{c.proofText}</p>}</div></section><section className="section-head compact"><div><span className="eyebrow"><Target size={15} />{c.learningMap}</span><h2>{c.total}: {Math.round((completed / totalLessons) * 100)}%</h2></div></section><div className="map-grid">{tracks.map((track) => <article key={track.id} className="map-card"><div><span className="track-dot" style={{ background: track.color }} />{track.title[language]}</div><strong>{getTrackProgress(track, progress)}%</strong><div className="progress-line"><span style={{ width: `${getTrackProgress(track, progress)}%`, background: track.color }} /></div></article>)}</div><p className="passport-note"><GitBranch size={16} />{language === 'ar' ? 'أفضل إثبات للمهارة هو مشروع قابل للمراجعة مع README واضح واختبارات ورابط نشر.' : 'The strongest proof of skill is a reviewable project with a clear README, tests, and a live link.'}</p></> }
